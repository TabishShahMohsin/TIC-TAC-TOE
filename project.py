...
def main(grid_width, winning_length):
    ...
    for i in range(winning_length):
        for j in range(winning_length):
            printing_grid.append(i*grid_width+j+1)
...
def check_who_won(l, grid_width, winning_length):
...
def minimax(l, grid_width, winning_length):
    print(l)
    ...
    for i in range(len(l)-1):
        ...
        if l[i]==" ":
            ...
            if check_who_won(t, grid_width, winning_length)!=2:
                t[-1]=check_who_won(t, grid_width, winning_length)
                
            # If this move doesn't end the game: using recursion
            else:
                e=minimax(t, grid_width, winning_length)
...
def player_move(chance, grid, grid_width):
    # Preventing the user form entering into a filled block
    while True:
        
        inp=int(input(f"Enter from 1 to {grid_width*grid_width}: "))
